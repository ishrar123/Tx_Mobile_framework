...
public class MobileKeywords {
	...
	public static boolean switchContext() {
		...
		for (String contextName : contextNames) {
			if (contextName.contains("cloud")) {
				...
				LogUtil.infoLog(thisClass, "Webdriver is " + GlobalUtil.getMDriver().context(contextName));
	...
	public static void scrollDown() {
		...
		for (int i = 1; i <= 9; i++) {
			...
			if (GlobalUtil.mdriver.findElements(MobileBy.AccessibilityId("Add to Cart Add to Shopping Cart"))
					.size() > 0) {
				...
				LogUtil.infoLog(thisClass, "NOT FOUND");
			}
	...
	public static void scrollTillMobileElementDisplayed(By locator, int maxNumberOfScrolls)
